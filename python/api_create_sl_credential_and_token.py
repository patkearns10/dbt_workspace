...
# TODO: use environment variables for the values
data = {
  "id": None,
  "account_id": account_id,
  ...
  "state": 1,
  "name": "test-sl-credential-created-via-api",
  "project_id": project_id
}
...
try:
    run_job_resp.raise_for_status()
except Exception as e:
    print(e)

semantic_layer_credentials_id = json.loads(run_job_resp.content)['data']['id']
semantic_layer_credentials_name = json.loads(run_job_resp.content)['data']['name']

print(f"""
semantic_layer_credentials_id: {semantic_layer_credentials_id}
semantic_layer_credentials_name: {semantic_layer_credentials_name}
"""
)
...
data = {
  ...
  "state": 1,
  "account_id": account_id,
  "name": "test-sl-token-created-via-api",
  "permission_grants": [
    {
      "project_id": project_id,
...
# OUTPUT:
  ...
  #                                          'writable_environment_categories': []}],
  #           'state': 1,
  #           'token': '1E4Bc1DYaA-XXXXXXXXXXXX',
  #           'token_string': 'dbtc_XXXXXXXXXXX',
  #           'type': None,
  #           'uid': 'XXXXXXXXXXXX',
...
service_token_id = json.loads(run_job_resp.content)['data']['id']
service_token_name = json.loads(run_job_resp.content)['data']['name']
token_string = json.loads(run_job_resp.content)['data']['token_string']
