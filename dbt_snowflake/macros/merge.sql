...
{% macro default__get_merge_sql(target, source, unique_key, dest_columns, incremental_predicates=none) -%}
    ...
    {% if unique_key %}
        {# if unique_key is sequence and unique_key is not mapping and unique_key is string #} 
        {% if unique_key is sequence and unique_key is not mapping and unique_key is string %} {# include string as well here #}
        {# BEGIN override: for backwards compatiblity, convert command separated string(unique_key) to list  - by Babu Krishnasamy #}
            {% if unique_key is string %}
                {% set unique_key = unique_key.split(",") %}
            {% endif %}
        {# END override: for backwards compatiblity - by Babu Krishnasamy #}            
