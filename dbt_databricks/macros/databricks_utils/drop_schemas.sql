...
Example 1 - dry run of current database
    dbt run-operation drop_schemas    
    
Example 2 - drop any schemas in a given database that match a given schema_like string
    dbt run-operation drop_schemas --args '{database: my_database, dry_run: False, schema_like: "cool_schema_%" }'
    
Example 3 - full clean :)
    dbt run-operation drop_schemas --args '{dry_run: False}'
#}

{% macro drop_schemas(database=target.database, dry_run=True, schema_like=None, schema_not_like=None) %}
    {%- set msg -%}
        Starting drop_schemas...
          database:        {{database}} 
          dry_run:         {{dry_run}} 
    ...
    {{ log('\nGenerating drop_schemas queries...\n', info=True) }}
